...
class EMImbalanceAdapter(AbstractImbalanceAdapter):
    ...
    @staticmethod
    def _validate_class_label_support(class_label_support, n_classes):
        """Return a Boolean class-support mask, or ``None`` if unspecified."""
        if class_label_support is None:
            return None

        support = np.asarray(class_label_support)

        if support.ndim != 1 or support.shape[0] != n_classes:
            raise ValueError(
                "class_label_support must be a one-dimensional Boolean or 0/1 mask with one entry per class")

        if support.dtype != np.bool_ and not np.all(np.isin(support, [0, 1])):
            raise ValueError(
                "class_label_support must contain only Boolean or 0/1 values")

        support = support.astype(bool)

        if not np.any(support):
            raise ValueError("class_label_support must include at least one class")
        return support

    # valid_labels are only needed if calibration is to be performed
    # or if self.estimate_priors_from_valid_labels is True.
    # class_label_support, when supplied, is a Boolean or 0/1 mask in softmax
    # column order that identifies classes allowed in the target distribution.
    def __call__(self, tofit_initial_posterior_probs,
                       valid_posterior_probs,
                       valid_labels=None,
                       class_label_support=None):

        # if binary labels were provided, convert to softmax format for consistency
        ...
        class_label_support = self._validate_class_label_support(
            class_label_support=class_label_support,
            n_classes=softmax_initial_posterior_probs.shape[1])
        ...
        if (class_label_support is not None and
            np.any(valid_class_freq[class_label_support] <= 0)):
            raise ValueError(
                "Each class in class_label_support must have a positive validation prior")
        ...
        if (class_label_support is not None):
            # q is constrained to a probability distribution over the known
            # support, even if a custom initializer proposes negative weights.
            first_iter_class_freq = np.maximum(first_iter_class_freq, 0)
            first_iter_class_freq = np.where(
                class_label_support, first_iter_class_freq, 0)
            if (np.sum(first_iter_class_freq) == 0):
                raise ValueError(
                    "class_label_support has no positive initialization mass")
        ...
        while ((next_iter_class_freq is None
            ...
            if (class_label_support is not None):
                # This is the M-step constrained to the simplex induced by
                # the known target-label support.
                next_iter_class_freq = np.where(
                    class_label_support, next_iter_class_freq, 0)
                next_iter_class_freq = (
                    next_iter_class_freq/np.sum(next_iter_class_freq))
            if (self.verbose):
